...
import { getLanguage, label, statusLabels, walletTypeLabels, activityTypeLabels, localized, localizedError, validationMessage } from '../utils/i18n'
...
async function uploadToCloudinary(file:File,cloudName:string,apiKey:string,apiSecret:string):Promise<string>{
  if(!cloudName||!apiKey||!apiSecret) throw new AppError('CLOUDINARY_UPLOAD_FAILED','',500)
...
function mapWalletError(c:any,err:unknown){
  if(err instanceof WalletServiceError) return c.json({success:false,error:{code:err.code,message:localizedError(c, String(err.code), err.message)}},err.statusCode)
  if(err instanceof AppError) return c.json({success:false,error:{code:err.code,message:localizedError(c, String(err.code), err.message)}},err.statusCode)
  console.error('WITHDRAWAL_ROUTE_ERROR:',err); return c.json({success:false,error:{code:'INTERNAL_SERVER_ERROR',message:localizedError(c,"INTERNAL_SERVER_ERROR")}},500)
}
...
adminWithdrawRouter.get('/withdrawals',async c=>{try{const status=c.req.query('status'); if(status&&!['Pending','Approved','Rejected'].includes(status))return c.json({success:false,error:{code:'VALIDATION_ERROR',message:localizedError(c,"INVALID_WITHDRAWAL_STATUS")}},400); const lang=getLanguage(c); const base=`SELECT wr.id,wr.user_id,wr.amount,wr.status,wr.admin_id,wr.screenshot_path,wr.screenshot_path AS screenshot_url,wr.wallet_number,wr.wallet_type,wr.created_at,wr.updated_at,u.user_name,u.phone_number,COALESCE(cl.activity_name,'') AS activity_name,COALESCE(cl.activity_type,'') AS activity_type FROM withdrawal_requests wr JOIN users u ON wr.user_id=u.id LEFT JOIN clients cl ON cl.user_id=u.id`; const q=status?`${base} WHERE wr.status=?1 ORDER BY wr.created_at DESC`:`${base} ORDER BY wr.created_at DESC`; const rows=status?await c.env.DB.prepare(q).bind(status).all<WithdrawalWithClient>():await c.env.DB.prepare(q).all<WithdrawalWithClient>(); const withdrawals=rows.results.map(w=>({...w,status_label:label(statusLabels,w.status,lang),wallet_type_label:label(walletTypeLabels,w.wallet_type,lang),activity_type_label:label(activityTypeLabels,w.activity_type,lang)})); return c.json({withdrawals},200)}catch(e){return mapWalletError(c,e)}})
.patch('/withdraw/:id',async c=>{try{const {userId:adminId}=c.get('jwtPayload') as TokenPayload; const id=Number(c.req.param('id')); if(!Number.isInteger(id)||id<=0)return c.json({success:false,error:{code:'VALIDATION_ERROR',message:localizedError(c,"INVALID_WITHDRAWAL_ID")}},400); const body=await c.req.parseBody(); const status=String(body.status||''); const image=body.screenshot; if(status!=='Approved'&&status!=='Rejected')return c.json({success:false,error:{code:'VALIDATION_ERROR',message:localizedError(c,"INVALID_WITHDRAWAL_STATUS")}},400)
  if(status==='Approved'){
    if(!(image instanceof File)||image.size===0)return c.json({success:false,error:{code:'VALIDATION_ERROR',message:localizedError(c,"PAYMENT_SCREENSHOT_REQUIRED")}},400)
    if(image.size>MAX_IMAGE_BYTES)return c.json({success:false,error:{code:'VALIDATION_ERROR',message:localizedError(c,"IMAGE_TOO_LARGE")}},400)
   ...
   if(!valid)return c.json({success:false,error:{code:'VALIDATION_ERROR',message:localizedError(c,"INVALID_IMAGE_TYPE")}},400)
